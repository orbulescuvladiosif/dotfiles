...
Claude Code: `~/.claude/`. Cursor: project-scoped rules and skills into a git repo you choose — matches how Cursor project config works; personal rules and skills stay local via `.git/info/exclude` (per repo) and the machine global gitignore (`~/.config/git/ignore`, or `core.excludesfile` if set).
